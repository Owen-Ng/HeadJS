...
(function(global){
	...
	*/
	...
	function dragdrop(){
		...
		const dragging = this._self.dragging;
		if (dragging.speed.x !== 0 && dragging.speed.y !== 0){
			const updateposition = () => {
				const position ={
					x: dragging.oldposition.x + dragging.speed.x,
					y: dragging.oldposition.y + dragging.speed.y
				};
			
			dragging.speed.x *= 0.9;
	      	dragging.speed.y *= 0.9;
	      	dragging.oldposition = position;
	      	applychange(dragging.element, position);
      	if (Math.abs(Math.max(dragging.speed.x, dragging.speed.y)) > 0.1) {
